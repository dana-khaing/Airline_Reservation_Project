defmodule AlbertAirlineWeb.UserLive.Registration do
  ...
  def mount(_params, _session, %{assigns: %{current_scope: %{user: user}}} = socket)
    ...
    {:ok, redirect(socket, to: AlbertAirlineWeb.UserAuth.signed_in_path(socket))}
  end

  def mount(_params, _session, socket) do
    changeset = Accounts.change_user_email(%User{}, %{}, validate_unique: false)

    {:ok, socket |> RateLimit.assign_client_ip() |> assign_form(changeset),
     temporary_assigns: [form: nil]}
  end

  @impl true
  def handle_event("save", %{"user" => user_params}, socket) do
    case RateLimit.check("register:#{socket.assigns.client_ip}", :timer.minutes(1), 5) do
      :ok ->
